...
nt.cost<-as.numeric() 	#empty C vector
nh.cost<-as.numeric() 	#empty N vector
Tissue<-as.numeric()	#empty Tissue vector
Height<-as.numeric()	#empty Height vector

#loop parameters
nt.int<-0.01		#Interval to increase C in loop
nh.int<-0.01		#Interval to increase N in loop
nt.max<-.5		#Max C in loop,
nt.min<-0.01		#Min C in loop

nh.max<-.5		#Max N in loop,
nh.min<-.01		#Min N in loop
nt<-nt.min		#start Rc @ Rc.min		
nh<-nh.min		#start Rn @ Rn.min
...
while (nt<nt.max) {
	...
	while (nh<nh.max) {
		nh<-nh+nh.int
		params<- c(Rc=250, Rn=50, zt=1.5, zh=1.5, ctc=1.5, chc=1.5, ctn=nt, chn=nh, alph=alpha, beta=(1-alpha)) 
		nt.cost<-c(nt.cost, nt)
		nh.cost<-c(nh.cost,nh)
...
out<-data.frame(nt.cost, nh.cost, Tissue, Height)
...
#plot results
dev.new()
scatterplot3d(x = nt.cost, y = nh.cost, z = Total, highlight.3d=TRUE)
dev.new()
scatterplot3d(x = nt.cost, y = nh.cost, z = Tissue, highlight.3d=TRUE)
dev.new()
scatterplot3d(x = nt.cost, y = nh.cost, z = Height, highlight.3d=TRUE)
